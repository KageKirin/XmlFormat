...
public class NamedAttributeParserTest
{
    [Theory]
    ...
    public void TestNamedAttributeNoValue(string input)
    {
        var result = XmlTokenParser.NamedAttribute(input)(new TextSpan(input));
    ...
    public void TestNamedAttributeEmptyValue(string input, string expected)
    {
        var result = XmlTokenParser.NamedAttribute(expected)(new TextSpan(input));
        Console.WriteLine($"parsing: `{input}`\nresult: {result}");
        Assert.Null(result.ErrorMessage);
        Assert.True(result.HasValue);
    ...
    [InlineData("at:tri-but_e=\"{hogehoge}\"", "at:tri-but_e", "{hogehoge}")]
    [InlineData("attribute0=\"{hogehoge}\"", "attribute0", "{hogehoge}")]
    public void TestNamedAttribute(string input, string expectedName, string expectedValue)
    {
        var result = XmlTokenParser.NamedAttribute(expectedName)(new TextSpan(input));
        Console.WriteLine($"parsing: `{input}`\nresult: {result}");
        Assert.Null(result.ErrorMessage);
        Assert.True(result.HasValue);
